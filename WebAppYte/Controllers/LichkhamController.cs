...
namespace WebAppYte.Controllers
{
	public class LichkhamController : Controller
	{
		...
		public ActionResult Dangxuly(int? id, int? page)
		{
			...
			LichKham ls = new LichKham();
			var lichKhams = ls.DSLichKham().Where(h => h.mabn == id && h.trangthai == 0)
				.OrderByDescending(x => x.ngaydat).ThenBy(x => x.madat);
			int pageSize = 3;
			int pageNumber = (page ?? 1);
			ViewBag.id = id;
		...
		public ActionResult Create(string hinhthuc, string chinhanh, string khoa, string bacsi, string ngaykham, string cakham, string mota)
		{
			...
			if (LKSave.chinhanh != null && LKSave.khoa != null && LKSave.bacsi != null && LKSave.hinhthuc != null && LKSave.ngaykham != null && LKSave.cakham != null && LKSave.mand != null && mota != null)
			{
				...
				// Kiểm tra nếu maca hợp lệ và tồn tại trong bảng CaKham
				bool isValidMaca = db.CaKhams.Any(x => x.maca == LKSave.maca);
				if (!isValidMaca)
				{
					ViewBag.Fail = "Ca khám này không tồn tại!";
					return PartialView();
				}
				...
				if (u != null)
				{
					...
					if (mota != null)
					{
						...
						if (ModelState.IsValid)
						{
							...
							{
								ViewBag.Fail = "Có lỗi xảy ra khi lưu dữ liệu: " + ex.Message;
								return PartialView();
							}
						}
					}
		...
		//	ViewBag.bs = bacsi;
		//	DateTime a = DateTime.Now;
		//	var ngay = db.CaKhams.ToList();

		//	LichKham lichkham = new LichKham();
		//	ViewBag.ngaykham = (from p in ngay
		//						where p.ngaykham.HasValue
		//						select p.ngaykham.Value.ToString("dd-MM-yyyy")).ToList();
		...
		//	f.ngaydat = DateTime.Now;
		//	f.trangthai = 0;
		//	if (LKSave.chinhanh != null && LKSave.khoa != null && LKSave.bacsi != null && LKSave.hinhthuc != null && LKSave.ngaykham != null && LKSave.cakham != null && LKSave.mand > 0 && mota != null)
		//	{
		//		string bs = (LKSave.ngaykham + "," + LKSave.cakham).ToString();
		//		LKSave.maca = lichkham.FindMaCa(bs, LKSave.hinhthuc, LKSave.bacsi);
		//		f.maca = LKSave.maca;
		...
		//				if (ModelState.IsValid)
		//				{
		//					int check = db.DatLiches.Count(x => x.maca == LKSave.maca);
		//					if (check == 0)
		//					{
		//						db.DatLiches.Add(f);
		//						db.SaveChanges();
		...
		//		//return Json(new { success = true, redirectUrl = Url.Action("Create", new { hinhthuc, chinhanh, khoa, bacsi, ngaykham, cakham, mota }) });
		//	}
		//	catch (Exception ex)
		//	{
		//		// Log lỗi
		...
		public async Task<JsonResult> getNgayKham(int? mand)
		{
			try
			{
				if (mand == null)
				{
					return Json(new { success = false, message = "Thiếu mã bác sĩ" }, JsonRequestBehavior.AllowGet);
				}

				var ngayKhams = db.CaKhams
								  .Where(x => x.mand == mand && x.ngaykham >= DateTime.Now)
								  .Select(x => x.ngaykham)
								  .Distinct()
								  .ToList();
		...
		public async Task<JsonResult> getCaKham(int? mand, string ngaykham)
		{
			try
			{
				if (mand == null || string.IsNullOrEmpty(ngaykham))
				{
					return Json(new { success = false, message = "Thiếu thông tin bác sĩ hoặc ngày khám" }, JsonRequestBehavior.AllowGet);
				}

				// Parse từ format dd-MM-yyyy
				DateTime parsedNgayKham = DateTime.ParseExact(ngaykham, "dd-MM-yyyy", CultureInfo.InvariantCulture);

				var caKhams = db.CaKhams
								.Where(x => x.mand == mand && x.ngaykham == parsedNgayKham)
								.Select(x => x.ca)
								.Distinct()
								.ToList();

				return Json(caKhams, JsonRequestBehavior.AllowGet);
			}
			catch (Exception ex)
			{
				return Json(new { success = false, message = ex.Message }, JsonRequestBehavior.AllowGet);
