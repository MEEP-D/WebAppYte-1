		...
		public int FindMaCa(string bs, string hinhthuc, string bacsi)
		{
			...
			using (modelWeb db = new modelWeb())
			{
				...
				var result = db.CaKhams.FirstOrDefault(y =>
					y.ngaykham.HasValue &&
					DbFunctions.TruncateTime(y.ngaykham.Value) == ngayParsed.Date && // So sánh phần ngày
					y.ca == ca &&
					y.hinhthuc == hinhthuc &&
					y.mand == bacsiInt);
				...
				if (result == null)
				{
					// Trả về thông báo lỗi nếu không tìm thấy ca khám phù hợp
					throw new Exception("Không tìm thấy ca khám phù hợp.");
				}

				return result.maca; // Trả về mã ca khám nếu tìm thấy
			}
